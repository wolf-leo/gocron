...
type TaskLog struct {
	...
	Hostname   string       `json:"hostname" xorm:"json NOT NULL"`                  // RPC主机名，JSON存储
	...
	AdminId    int          `json:"admin_id" xorm:"int notnull index default 9999"` // 管理员id 默认9999 为系统自动运行
	BaseModel  `json:"-" xorm:"-"`
}
...
func (taskLog *TaskLog) parseWhere(session *xorm.Session, params CommonMap) {
	...
	adminId, ok := params["AdminId"]
	if ok && adminId.(int) > 0 {
		session.And("admin_id = ?", adminId)
	}
}
